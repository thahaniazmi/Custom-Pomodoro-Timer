// Cloudflare Worker entrypoint with Static Asset fallback
export default {
  async fetch(request, env) {
    const url = new URL(request.url);

    // API Route: Send Email OTP via Resend
    if (request.method === 'POST' && url.pathname === '/api/send-otp') {
      try {
        ...
        if (!apiKey) {
          return new Response(JSON.stringify({ error: 'RESEND_API_KEY is not configured in Cloudflare Workers environment variables.' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' }
          });
        }
